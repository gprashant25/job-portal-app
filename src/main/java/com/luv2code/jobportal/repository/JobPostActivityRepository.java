...
public interface JobPostActivityRepository extends JpaRepository<JobPostActivity, Integer> {
    ...
    // Adding a supporting method additional: when we display all of our jobs on the recruiter dashboard, it'll show information about the job, also tells us how many people have applied for that given job.
    // BELOW QUERY MEANS: 1) Retrieves list of jobs for a given recruiter id. 2) Combines/joins job post activity, job location, company. 3) Also includes a count of job seekers/candidates who have applied for a given job.
    ...
    List<JobPostActivity> search(@Param("job") String job,
                                 @Param("location") String location,
                                 @Param("remote") List<String> remote,
                                 @Param("type") List<String> type, @Param("date") LocalDate date);
}
